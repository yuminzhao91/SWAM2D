...
dfig1 = np.fromfile('validation_fsurf_test/snapp00801', dtype=np.float32)
dfig1 = dfig1.reshape(481, 481).swapaxes(1,0)

dfig2 = np.fromfile('validation_fsurf_test/snapp01201', dtype=np.float32)
dfig2 = dfig2.reshape(481, 481).swapaxes(1,0)

dfig3 = np.fromfile('validation_fsurf_test/snapp01601', dtype=np.float32)
dfig3 = dfig3.reshape(481, 481).swapaxes(1,0)

dfig4 = np.fromfile('validation_fsurf_test/snapp02001', dtype=np.float32)
...
font1.savefig('validation_snap_fsurf.ps', transparent=True)
sp.call('ps2eps -l -B -s b0 -c -n -f validation_snap_fsurf.ps', shell=True)
sp.call('rm -f validation_snap_fsurf.ps', shell=True)

# >> TRACES

tmp1 = np.loadtxt('validation_fsurf_test/Uz_file_an')
tmp2 = np.fromfile('validation_fsurf_test/recz.bin', dtype=np.float32)
...
tmp1x = np.loadtxt('validation_fsurf_test/Ux_file_an')
tmp2x = np.fromfile('validation_fsurf_test/recx.bin', dtype=np.float32)
...
font1.savefig('validation_trac_fsurf.ps', transparent=True)
sp.call('ps2eps -l -B -s b0 -c -n -f validation_trac_fsurf.ps', shell=True)
sp.call('rm -f validation_trac_fsurf.ps', shell=True)
