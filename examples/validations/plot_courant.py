...
dfig1 = np.fromfile('validation_courant/snapp00051', dtype=np.float32)
dfig1 = dfig1.reshape(481, 481).swapaxes(1,0)

dfig2 = np.fromfile('validation_courant/snapp00101', dtype=np.float32)
dfig2 = dfig2.reshape(481, 481).swapaxes(1,0)

dfig3 = np.fromfile('validation_courant/snapp00151', dtype=np.float32)
dfig3 = dfig3.reshape(481, 481).swapaxes(1,0)

dfig4 = np.fromfile('validation_courant/snapp00201', dtype=np.float32)
dfig4 = dfig4.reshape(481, 481).swapaxes(1,0)

font1 = plt.figure(figsize=(6.,9.0))
...
fig1.text(5., 195., r"\bf t=0.05s", fontsize=10)
...
fig2.text(5., 195., r"\bf t=0.1s", fontsize=10)
...
fig3.text(5., 195., r"\bf t=0.15s", fontsize=10)
...
fig4.text(5., 195., r"\bf t=0.2s", fontsize=10)
...
font1.savefig('validation_courant.ps', transparent=True)
sp.call('ps2eps -l -B -s b0 -c -n -f validation_courant.ps', shell=True)
#sp.call('rm -f validation_courant.eps', shell=True)
#sp.call('ps2eps -l -B -s b0 -F validation_courant.ps', shell=True)
#sp.call('ps2epsi validation_courant.ps validation_courant.eps', shell=True)
sp.call('rm -f validation_courant.ps', shell=True)
