...
dfig1 = np.fromfile('validation_water/snapp04001', dtype=np.float32)
dfig1 = dfig1.reshape(481, 481).swapaxes(1,0)

dfig2 = np.fromfile('validation_water/snapp06001', dtype=np.float32)
dfig2 = dfig2.reshape(481, 481).swapaxes(1,0)

dfig3 = np.fromfile('validation_water/snapp08001', dtype=np.float32)
dfig3 = dfig3.reshape(481, 481).swapaxes(1,0)

dfig4 = np.fromfile('validation_water/snapp09981', dtype=np.float32)
...
fig1.imshow(dfig1, cmap='gray', aspect='equal', extent=[-20,220,220,-20], vmin=-5.0e-4, vmax=5.0e-4)
...
fig2.imshow(dfig2, cmap='gray', aspect='equal', extent=[-20,220,220,-20], vmin=-5.0e-4, vmax=5.0e-4)
...
fig3.imshow(dfig3, cmap='gray', aspect='equal', extent=[-20,220,220,-20], vmin=-5.0e-4, vmax=5.0e-4)
...
fig4.imshow(dfig4, cmap='gray', aspect='equal', extent=[-20,220,220,-20], vmin=-5.0e-4, vmax=5.0e-4)
fig4.plot(pmlbox[:,0], pmlbox[:,1], linestyle=':', linewidth=1, color="black")

font1.savefig('validation_snap_water.ps', transparent=True)
sp.call('ps2eps -l -B -s b0 -c -n -f validation_snap_water.ps', shell=True)
sp.call('rm -f validation_snap_water.ps', shell=True)
